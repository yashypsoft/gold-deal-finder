            ...
            async startNewScan() {
                ...
                try {
                    ...
                    this.showNotification(
                        'success',
                        `🔄 Scan started! Found ${count} products`
                    );
                    ...
                    if (error.response?.status === 429) {
                        this.showNotification(
                            'warning',
                            error.response.data?.detail || 'Scan recently triggered. Try later.'
                        );
                        return;
                    }
                    ...
                    this.showNotification(
                        'error',
                        error.response?.detail || 'Failed to start scan'
                    );
